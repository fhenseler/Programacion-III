<?php
public static function GuardarArchivo($origen, $destino)
{
    $backup_path = './backup/';
    $backup = $backup_path . $origen;

    if(file_exists($destino))
    {
        if(move_uploaded_file($_FILES['test_imagen']['tmp_name'], $backup))
        {
            echo 'The file ' . $backup . ' has been uploaded to backup.' . '<br>';
        }
        else
        {
            echo 'ERROR!';
    ...
    {
        if(move_uploaded_file($_FILES['test_imagen']['tmp_name'], $destino))
        {
            echo 'The file ' . $destino . ' has been uploaded.' . '<br>';
        }
        else
        {
            echo 'ERROR!';
    ...
    /*ARREGLAR Y TERMINAR, VER COPY Y MOVE_UPLOAD_FILE*/
}
